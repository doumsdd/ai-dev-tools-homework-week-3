# Agent Relay - Questions & Answers Summary

This document summarizes all the questions and answers from the project implementation.
...
| **`postgres`** | ✅ | **The service name - Docker resolves this to the postgres container's IP** |
...
postgresql://relay:relay@postgres:5432/agent_relay
             ↑         ↑    ↑        ↑     ↑
          protocol   user pass  hostname port  db
                               ↑
...
- **Kubernetes Deployment**: See `KUBERNETES_DEPLOYMENT.md` and `K8S_SUMMARY.md`
- **CI/CD Pipeline**: See `CI_CD_PIPELINE.md` and `CI_CD_SUMMARY.md`
